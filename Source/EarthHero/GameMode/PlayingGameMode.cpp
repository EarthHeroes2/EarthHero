...
void APlayingGameMode::AddPlayerDead(AEHPlayerController* DeadEHPlayerController, FVector DeadLocation)
{
	...
	if(DeadEHPlayerController)
	{
		...
		if(World)
		{
			...
			ACustomSpectatorPawn* CustomSpectatorPawn = World->SpawnActor<ACustomSpectatorPawn>(ACustomSpectatorPawn::StaticClass(), DeadLocation + FVector(0.f, 0.f, 300.f), DeadLocation.Rotation(), SpawnParams);
...
{
	UE_LOG(LogTemp, Log, TEXT("APlayingGameMode::Rebirth(AEHPlayerController* EHPlayerController)"));
